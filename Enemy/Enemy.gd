...
var initial_posittion = Vector2.ZERO
...
func _ready():
	initial_posittion.x = -100
	initial_posittion.y = y_positions[randi() % y_positions.size()]
	position = initial_posittion

func _physics_process(_delta):
	position += direction
	position.y = initial_posittion.y + sin(position.x/20)*wobble
	if position.x >=1200:
		queue_free()
